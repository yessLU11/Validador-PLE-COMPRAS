...
# ============================================================================
# excel_reader: Lee hojas de Excel y normaliza datos
# database: Gestiona la BD SQLite (crear, insertar, consultar, eliminar)
# validator: Detecta duplicados comparando registros
# report_generator: Crea reportes Excel con resultados
...
# ============================================================================
# Define la apariencia y comportamiento básico de la aplicación
st.set_page_config(
...
# ============================================================================
# Define toda la apariencia visual: colores, espaciados, efectos, etc.
...
<style>
    ...
    /* ===== TARJETAS (CARDS) ===== */
    /* Contenedores visuales con sombra sutil */
    .card {
    ...
    /* Efecto hover en cards: sombra más pronunciada */
    .card:hover {
    ...
    /* ===== ESTADÍSTICAS ===== */
    /* Cajas de estadísticas - Colores llamativos con degradado */
    .stat-box {
    ...
    /* Etiqueta descriptiva en estadísticas */
    .stat-label {
        font-size: 0.85rem;
        opacity: 0.9;
    }
    ...
    /* Estilos para botones (hereda de Streamlit) */
    .stButton button {
        border-radius: 0.5rem;
        font-weight: 500;
...
# ============================================================================
# Mostrar título en sidebar y en área principal
with st.sidebar:
...
# ============================================================================
# Divide la interfaz en 3 secciones:
# 1. Validación - Flujo principal
# 2. Instrucciones - Guía detallada
...
# ============================================================================
# PESTAÑA 1: VALIDACIÓN (RESULTADOS Y ACCIONES)
# ============================================================================
with tab_validar:
    if st.session_state.df_nuevo is None:
        ...
        # ===== Panel de información: Mes, Registros, Estado =====
        ...
        if st.session_state.duplicados is not None:
            ...
            if st.session_state.duplicados.empty:
                # SIN duplicados - se puede agregar al historial
                st.success("🎉 **Sin duplicados** – Este mes está listo para agregar al historial")
            else:
                # CON duplicados - mostrar resumen, detalle y botón de descarga
                ...
                # 2. Mostrar detalle (primeros 20 registros duplicados)
                ...
                # 3. Botón de descarga del reporte Excel completo
            ...
            if st.session_state.duplicados is not None:
                ...
                if st.button("✅ CONFIRMAR Y AGREGAR AL HISTORIAL", use_container_width=True, type="primary"):
                    try:
                        ...
                        # Reexecutar app para actualizar la interfaz
                        st.rerun()
                    except Exception as e:
                        # Capturar y mostrar errores en BD
...
# ============================================================================
# PESTAÑA 2: GUÍA DE INSTRUCCIONES (MANUAL DEL USUARIO)
# ============================================================================
with tab_instrucciones:
    ...
    with st.expander("🟢 PASO 1: Preparar el archivo", expanded=True):
        ...
        - **Contenido**: Debe tener las hojas:
          - `8.1` (datos desde fila 8)
          - `PROGRAMAS SOCIALES` (datos desde fila 2)
...
with st.sidebar:
    ...
    # Muestra el estado actual: cuántos meses hay, cuáles son
    ...
    if meses:
        ...
        # Mostrar estadística: cuántos meses de los 12 máximos
        st.markdown(
    ...
    if archivo is not None:
        ...
        if not match:
            ...
            # Guardar archivo temporalmente en carpeta uploads/
            temp_path = f"uploads/{nombre}"
            with open(temp_path, "wb") as f:
                f.write(archivo.getbuffer())
            
            # Leer archivo (leer desde ambas hojas: 8.1 y PROGRAMAS SOCIALES)
            with st.spinner("Leyendo archivo..."):
                df = leer_hoja_unificada(temp_path, mes)
            
            if df.empty:
                # Error: no hay datos en las hojas especificadas
                st.error("❌ No hay datos en hojas 8.1 / PROGRAMAS SOCIALES")
            else:
                # Éxito: guardar en estado de sesión
    ...
    # ===== Botón de validación =====
    # Solo aparece si hay datos cargados (df_nuevo no es None)
        ...
        if st.button("🔍 VALIDAR DUPLICADOS", use_container_width=True, type="primary"):
            try:
                ...
                if dups.empty:
                    ...
                    # Con duplicados: generar resumen y reporte
                    st.warning(f"⚠️ {len(dups)} duplicados encontrados")
                    
                    # Generar resumen agrupado por mes y tipo de comprobante
                    ...
                    # Generar archivo Excel con los duplicados
                ...
                # Reexecutar app para mostrar resultados
                st.rerun()
            except Exception as e:
                # Capturar errores en el proceso de validación
                st.error(f"❌ Error en validación: {str(e)}")
    
    # ===== Botón de descarga adicional en sidebar =====
    # Redundancia para mejor UX (la principal está en la pestaña)
    if st.session_state.reporte_path and os.path.exists(st.session_state.reporte_path):
        try:
            with open(st.session_state.reporte_path, "rb") as f:
                st.download_button(
            ...
            st.warning(f"⚠️ No se pudo crear botón de descarga: {str(e)}")
    
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    
    # ===== SECCIÓN 3: Gestión avanzada =====
    # Botones para eliminar meses, cargar batch, etc.
    st.markdown('<p class="sidebar-title">⚙️ Gestión</p>', unsafe_allow_html=True)
    
    # Botón para eliminar el último mes agregado
    if st.button("🗑️ Eliminar último mes", use_container_width=True, help="Quita el mes más reciente agregado"):
        try:
            ...
            st.error(f"❌ Error al eliminar mes: {str(e)}")
    
    # Expandible para carga de múltiples archivos (batch)
    with st.expander("📂 Cargar múltiples archivos"):
        ...
        if archivos_batch and st.button("Cargar", key="btn_cargar_batch"):
            try:
                # Preparar lista de archivos temporales
                temp_paths = []
                ...
                st.error(f"❌ Error en carga batch: {str(e)}")
    
    # Botón para eliminar TODA la base de datos (IRREVERSIBLE)
        ...
        with col1:
            if st.button("Sí, eliminar", key="btn_confirmar_eliminar"):
                try:
                    ...
                    st.error(f"❌ Error al eliminar BD: {str(e)}")
        
        with col2:
            st.button("Cancelar", key="btn_cancelar")


# ============================================================================
# PIE DE PÁGINA
# ============================================================================
# Información de versión, créditos y nota de seguridad
st.markdown("---")
